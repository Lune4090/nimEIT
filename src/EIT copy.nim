...
func generate_system(electrodes: seq[OuterNode]): Result[Mesh, CatchableError] =
  ...
  for i in 0..<len(outer_nodes):
    meshes.add(Element(idNode1: 0, idNode2: i mod len(outer_nodes), idNode3: (i+1) mod len(outer_nodes), isNode1Outer: false, isNode2Outer: true, isNode3Outer: true))

  return Mesh(meshes: meshes, outerNodes: outer_nodes, innerNodes: inner_nodes).ok()
...
func add_new_node_to_system(system: var Mesh, newNode: InnerNode) =
  ...
  var meshesUpdated: seq[(int, Element)]
  let numNewNode = len(system.innerNodes)

  # 1. メッシュ毎に外接円の中心を算出し、更新対象に入るか(= 新ノードを外接円内に含むか)を確認
  for i, mesh in system.elements.pairs():
    var
  ...
  for (i, mesh) in meshesUpdated.items():
    system.elements.delete(i-deletedNum)
    deletedNum += 1
...
proc draw_system(system: Mesh) = 
  ...
  for mesh in system.elements.items():
    let
...
proc fem(system: Mesh): Tensor[float] =
  ## 各ノード毎に連立方程式を立式
...
func calc_area(tensor1: Tensor[float], tensor2: Tensor[float]): float = 
  return 0.5*abs(tensor1[0]*tensor2[1]-tensor1[1]*tensor2[0])

proc k_ij_tri(xy: Tensor[float]): Tensor[float] =
  ## 三角形エレメントにおける剛性行列K_ijの計算
  ## https://github.com/eitcom/pyEIT/blob/master/pyeit/eit/fem.py

  let
    edges = @[xy[2], xy[0], xy[1]].toTensor.reshape(2, 3) - @[xy[1], xy[2], xy[0]].toTensor.reshape(2, 3) 
    area = calc_area(edges[0, _].reshape(2), edges[1, _].reshape(2))
  
  return edges*edges.transpose / (4.0*area)

# 最早outerとinnerを分ける必要性がない可能性が出てきたので、全部書き換えるかもしれない
func calc_ks_tri(system: Mesh): Tensor[float] =
  ## 三角形エレメント全体に対する局所剛性行列の計算
  let ks_tri = zeros[float]([3, len(system.innerNodes) + len(system.outerNodes), len(system.innerNodes) + len(system.outerNodes)])
